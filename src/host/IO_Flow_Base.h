...
#define NVME_UPDATE_SQ_TAIL(Q)  Q.Submission_queue_tail++;\
	...
	public:
		...
		uint32_t Get_min_end_to_end_request_delay();//in microseconds
			...
			void Set_dependency_manager(Request_Dependency_Manager* manager) { dependency_manager = manager; }
			std::uint64_t Get_read_bytes() const { return STAT_transferred_bytes_read; }
			std::uint64_t Get_write_bytes() const { return STAT_transferred_bytes_write; }
			std::uint64_t Get_trim_bytes() const { return STAT_trimmed_bytes; }
			std::uint64_t Get_measurement_write_bytes() const { return STAT_measurement_write_bytes; }
			void Set_measurement_window(sim_time_type start_time, sim_time_type end_time)
			{
				measurement_start_time = start_time;
				measurement_end_time = end_time;
			}
			void Set_pool_id(const std::string& value) { pool_id = value; }
			uint16_t Get_flow_id() const { return flow_id; }
	...
	protected:
		...
		LHA_type start_lsa_on_device, end_lsa_on_device;
			...
			void Record_dependency_wait(sim_time_type wait_time);
		...
		std::uint64_t STAT_measurement_write_bytes;
		sim_time_type measurement_start_time, measurement_end_time;
		sim_time_type STAT_dependency_wait_time_total, STAT_dependency_wait_time_max;
